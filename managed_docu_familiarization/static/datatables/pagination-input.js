...
$.fn.DataTable.ext.renderer.pageButton = $.extend(true, $.fn.DataTable.ext.renderer.pageButton,
    {
        bootstrap: function (settings, host, idx, buttons, page, pages) {
            ...
            input_html = '<div class="input-group input-group-sm">' +
                '<input style="width: ' + (Math.ceil(Math.log10(pages + 1)) + 2.5) + 'em; margin-left: -1px;" ' + (pages === 1 ? 'disabled ' : '') + 'class="form-control" type="number" min="1" max="' + pages + '">' +
                '<span class="input-group-text" id="basic-addon2"> of ' + pages + '</span>' +
                '</div>'
